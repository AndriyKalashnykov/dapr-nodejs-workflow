...
- **docker** (tag pushes only): the entire job is gated to tag pushes (`startsWith(github.ref, 'refs/tags/')`) — the image build, Trivy image CVE scan (CRITICAL/HIGH blocking), boot-marker smoke test, multi-arch build, GHCR publish, and cosign signing all run solely on tags. Regular pushes/PRs skip it; per-push functional coverage (and the amd64 image build) comes from `e2e`/`e2e-dapr`. On the tag run: Gates 1–3 (single-arch build, Trivy image scan, boot-marker smoke via `make docker-smoke-test`), Gate 4 (multi-arch build via `docker/build-push-action`), then step-level `if: startsWith(github.ref, 'refs/tags/')` guards (now redundant with the job gate, kept as defense-in-depth) on `Log in to GHCR`, `push`, `Verify multi-arch manifest` via `make docker-verify-manifest`, `Install cosign`, and `Sign image with cosign` (Sigstore keyless OIDC signing by digest). Buildkit in-manifest attestations (`provenance`/`sbom`) are explicitly disabled to keep the image index clean of `unknown/unknown` platform entries so GHCR's Packages UI renders the "OS / Arch" tab.
