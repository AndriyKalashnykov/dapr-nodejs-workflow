...
Job dependencies: `changes` -> `static-check` -> `build` + `test` (parallel) -> `e2e-compose` + `e2e` + `e2e-dapr` + `e2e-durability` + `integration-test` + `dast` + `docker` (all seven parallel; `e2e-compose`/`e2e`/`e2e-dapr`/`e2e-durability`/`integration-test`/`dast` need `[changes, build, test]`, `docker` needs `[changes, static-check, build, test]`) -> `ci-pass` (lists every upstream job including `changes` in its `needs:`). `dast` and `docker` additionally gate on `startsWith(github.ref, 'refs/tags/')`, so on non-tag pushes/PRs they are **skipped** — `ci-pass` treats `skipped` as passing (it only fails on `failure`/`cancelled`), so it stays green.
